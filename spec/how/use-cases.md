...
| e | **Guest** | Log in as **`ai`**, then **`~/ai-sandbox/config/start-dev.sh`** (or **`start-day.sh`**) to open Cursor and the default project container. |
...
| c | **Guest** | **`~/ai-sandbox/config/start-day.sh`** or **`start-dev.sh`** — starts the default project’s container (detached) and launches **Cursor** on that workspace. For a specific project: **`~/ai-sandbox/config/start-container.sh <name>`**. |
...
| Returning | **`start-vm.sh`**, **`start-vm.ps1`**; **`stop-vm.sh`**, **`stop-vm.ps1`** | `start-vm-*`, `stop-vm-*` | **`start-day.sh`**, **`start-dev.sh`**, **`start-container.sh`** |
